>[!Info]
>本文旨在介紹 file system，不包括與 file system 相關的指令，若要查看與 file system 相關的指令，請見[本文](</Operating System/Shell/9 - 與 File System 相關的指令.md>)。
...
>[!Note]
>即使執行檔位在當前目錄，也須要在檔名前面加上 `./`，因為若單純輸入檔名，會被 shell 認為是一個 command，此時 shell 只會從 [PATH 這個環境變數](</Operating System/Shell/1 - Introduction.md#`PATH`>)中所列的 paths 中尋找執行檔，反而不會找當前的目錄。
